...
Their package [rrtools](https://github.com/benmarwick/rrtools) provides tools to automatically set up a new, sensibly structured directory which you can work in if you aim to conduct and disseminate research in a reproducible manner using R.
...
The documentation of a function is commonly set up using [roxygen2](https://roxygen2.r-lib.org), which generates a document (in the form of an .Rd file) that R will render when you call the manual page, provided it is located in the correct `man/` directory within the package directory itself (see <https://r-pkgs.org/structure.html> for more on how to structure a source package).
...
Here is an example given in the *R packages* book, where a simple function that adds two numbers together is documented using roxygen2 and then the function itself is defined in R code:
...
Parameters with default values which can be adjusted should also be listed here. After this is a description of what the function returns, followed by two examples of how the function can be used.
